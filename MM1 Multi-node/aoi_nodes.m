...
for b = 1:length(lambda)
    ...
    for k = 1:length(nodes)
        ...
        [ground_indices, final_arrival_times, departure_timestamps, waiting_times, buffer_lengths, largest_time] = first_node(num_users, lambda_users(1, :), mu_node, epsilon_node, num_events, num_events_considered);

        for i = 2:num_nodes
            mu_node = (num_users*i)*higher;
            [arrival_times_out, delay, arrival_timestamps_all, departure_timestamps_out, ground_indices_out, largest_time_out, buffer_lengths, waiting_times] = other_nodes(departure_timestamps, num_users, lambda_users(i, :), mu_node, epsilon_node, largest_time, final_arrival_times, ground_indices);
        ...
        av_age_nodes(b, k) = av_age_func(departure_timestamps(ground_indices'), final_arrival_times, length(final_arrival_times));
...
legend('3.5', '5', '7', '10', '15'); xlabel('Number of nodes'); ylabel('AoI');

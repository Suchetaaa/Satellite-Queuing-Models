function [ground_indices, final_arrival_times, departure_timestamps, waiting_times, buffer_lengths, largest_time] = first_node(num_users_periodic, num_users_poisson, lambda_periodic_users, offset_users, lambda_poisson_users, mu_node, epsilon_node, num_events, num_events_considered)
    % num_users = 10;
    ...
    for i = 1:num_users_poisson
        inter_event_times = 1/lambda_poisson_users(1, i)*log(1./rand(1,num_events));
        event_times_users(i+num_users_periodic, :) = cumsum(inter_event_times);
    end

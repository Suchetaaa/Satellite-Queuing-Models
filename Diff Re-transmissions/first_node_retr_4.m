%Represents the first node for with re-transmissions = 4
function [ground_indices, final_arrival_times, departure_timestamps, waiting_times, buffer_lengths, largest_time] = first_node_retr_4(num_users, lambda_users, offset_users, mu_node, epsilon_node, num_events, num_events_considered, max_retransmissions)
...
%     end
    ...
    random_indices_4_useful = rand(1, number_3) > epsilon_node;
    
    %Contains all those indices which require the fourth retransmission
    random_indices_4 = find(random_indices_4_useful == 1);
    number_4 = length(random_indices_4);
    
    %1 means transmission cannot occur, 0 means 4 retransmissions
    random_indices_5 = rand(1, number_4) > epsilon_node;

    for i = 1 : number_4 
        if (random_indices_5(i) == 1)
            missed(random_indices_1(random_indices_2(random_indices_3(random_indices_4(i))))) = 1;
        end
    end
    
    random_indices_4_actual = zeros(1, number_4);
    random_indices_3_actual = zeros(1, number_3);
    random_indices_2_actual = zeros(1, number_2);
    
    for i = 1:number_4
        %Contains the actual indices from the (1: num_events_considered) which require 4 retransmissions
        random_indices_4_actual(i) = random_indices_1(random_indices_2(random_indices_3(random_indices_4(i))));
        retransmissions(1, random_indices_1(random_indices_2(random_indices_3(random_indices_4(i))))) = 4;     
    end
    ...
    retransmissions_3_indices = setdiff(random_indices_3_actual, random_indices_4_actual);
    ...
    retransmissions(retransmissions_3_indices) = 3;
    retransmissions(retransmissions_2_indices) = 2;
    retransmissions(retransmissions_1_indices) = 1;
    retransmissions(random_indices_4_actual) = 4;
    
    %Assigning departure timestamps tp the packets
    server_timestamps(1) = offset;
    index_missing = 1;

    if (retransmissions(1) == 4)
        departure_timestamps(1) = server_timestamps(1) + inter_service_times(1) + inter_service_times(2) + inter_service_times(3) + inter_service_times(4) + inter_service_times(5);
        index_missing = index_missing + 4;
    elseif (retransmissions(1) == 3)
    ...
    for i = 2:num_events_considered
        if (retransmissions(i) == 4)
            ...
            departure_timestamps(i) = server_timestamps(i) + inter_service_times(i+index_missing-1) + inter_service_times(i+index_missing) + inter_service_times(i+index_missing+1) + inter_service_times(i+index_missing+2) + inter_service_times(i+index_missing+3);
            index_missing = index_missing + 4;
        elseif (retransmissions(i) == 3)
            if final_arrival_times(i) < departure_timestamps(i-1)
                server_timestamps(i) = departure_timestamps(i-1);
            else
                server_timestamps(i) = final_arrival_times(i);

%Returns simulated and theoretical average AoI for the zero-wait policy
%tandem queue ad described in the paper
function [av_age_simulation, av_age_theoretical] = zero_wait(rho)
%Rho is basically defined as mu_node_1/mu_node_2 as given in the paper
    ...
    for j = 1 : length(rho)
        ...
        %Arrival timestamps of the first node
        arrival_timestamps = cumsum(inter_service_times(1, 1:num_events-1));
        arrival_timestamps = [0 arrival_timestamps];

        %Departure timestamps of the packets from node 1 OR arrival
        %timestamps of the packets to node 2 following zero-wait policy
        departure_timestamps = cumsum(inter_service_times);

        %Inter-service times for node 2
        inter_service_times_2 = 1/mu_node_2*log(1./rand(1, num_events));

        %Generating final departure timestamps
